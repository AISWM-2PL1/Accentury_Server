...
public record AnalysisStatusResponse(long pollAfterMs, List<Item> items) {
    ...
    /**
     * 문항 하나의 대표 상태. 시도가 여럿이면 이렇게 접는다 (2026-08-10 확정):
     * 최신 시도가 분석 중이면 PROCESSING(새 결과가 채점 대상을 갈아치울 수 있어 대기),
     * 아니면 성공 시도가 하나라도 있으면 COMPLETED(채점 대상이 있으니 재녹음 불필요),
     * 그것도 없으면 최신 시도의 실패 상태다. /complete의 "최신 성공 시도 1건이면 완료"
