 ...
 * 하나를 최대 {@code ai-timeout x (재전송 + 1)}만큼 붙들고(타임아웃은 재전송하지 않지만 5xx는
 * 한다 - KAN-172), 워커({@code dispatch-concurrency}, 실모델은 1)가 다 차면 전달 큐가 밀려
 ...
 *
 * <h4>인스턴스별 회로 - 의도된 것이다 (KAN-167)</h4>
 * backend가 Fargate 태스크 여러 개로 돌아도(KAN-165, KAN-168) 회로 상태는 프로세스 메모리에
 * 두고 공유하지 않는다. 회로는 "이 프로세스가 AI를 불러 본 결과"를 요약한 것이라 각자 관찰하는
 * 것이 표준 관행이다 - 한 태스크의 네트워크 경로만 끊긴 장애에서 다른 태스크까지 함께 닫히면
 * 멀쩡한 경로의 사용자까지 503을 받는다. 비용은 태스크 수만큼의 독립 판정이다: AI가 실제로
 * 죽으면 태스크마다 연속 실패 임계치를 따로 채우고(최대 3배의 실패 요청), 복구 시험도 태스크마다
 * 1건씩 나간다. 경보(ai-circuit-open)는 게이지의 Maximum 통계라 어느 한 태스크만 열려도 드러난다.
 * Redis 같은 공유 저장소로 옮길 조건은 {@code RateLimits}에 적은 것과 같다.
 */
