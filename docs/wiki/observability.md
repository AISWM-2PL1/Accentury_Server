...
닫힌 목록으로 좁혀 둔 이유가 이것이다 - 전달본이 모르는 이름을 하나 보낼 때마다 조합이 늘어
요금이 상한 없이 자란다 (`ai/app/stages.py`가 목록에 없는 이름을 버린다).
...
| `StageDuration` | 추론 단계별 소요 시간(ms). 차원 `stage`와 `warm`이 더 붙는다 (KAN-204, 아래) |
...
문항당 처리 시간이 어느 단계에 쓰였는지가 로그에도 지표에도 없었다. GPU 전환(KAN-57)은 Whisper만
줄이고 MFA는 CPU 작업이라 그대로인데 두 단계의 비율을 몰랐고, MFA 상주화도 인스턴스 상향(KAN-36)도
같은 자리였다. `StageDuration` 하나에 차원 `stage`와 `warm`을 붙여 그 분포를 낸다.

| `stage` | 무엇을 잰 것인가 | 누가 재는가 |
|---|---|---|
| `lockWait` | 앞 요청의 추론이 끝나기를 기다린 시간 | 어댑터 (`ai/app/track1.py`) |
| `workerLoad` | 워커 적재와 재적재 대기. 적재가 실제로 있었던 요청에만 표본이 생긴다 | 어댑터 |
| `model` | 전달본 `Track1Scorer.score` 호출 전체 | 어댑터 |
| `transcribe` `gate` `align` `f0` `scoring` | 전사, 내용 게이트, 정렬, F0 추출, 거리와 채점 | 전달본 |
| `total` | 엔진 호출 전체 (합계) | 라우트 (`ai/app/analyze.py`) |

`warm`은 `cold`와 `warm` 둘이고, 콜드는 **그 워커의 첫 채점**이라는 뜻이다. 가중치는 워밍업에서
이미 올라와 있어도 전달본 안쪽의 지연 초기화가 첫 채점에서 한 번 일어난다 - staging 첫 호출
22.9초와 그 뒤 10초대의 차이(KAN-172 실측)가 그것이고, 그 오버헤드를 지표에서 가르는 것이 이
차원이다.

**전달본 몫의 다섯은 아직 비어 있다.** `score()` 안쪽은 어댑터에서 잴 수 없어 워커 결과 JSON의
`stageMs`(단계 이름 -> ms)로 받기로 인터페이스만 정해 두었다. 전달본이 그 값을 싣기 시작하면
어댑터도 대시보드도 고칠 것이 없다.

**백분위가 여기서는 진짜 백분위다.** 호스트 타이머가 관측값을 낱개로 올리므로(`Values`)
CloudWatch가 전체 표본에서 p50과 p95를 계산한다 - backend 쪽이 "가장 나쁜 태스크의 P95"로
타협한 것과 다르다.

`GET /internal/v0/metrics/stages`는 **읽으면 비워진다.** 소비자가 호스트 타이머 하나라는 전제이고,
그래서 그 회차의 `put-metric-data`가 실패하면 그 1분의 표본은 사라진다. 지표는 최선 노력이고,
재시도 큐를 두면 CloudWatch가 오래 막힌 동안 추론 프로세스가 그 대가를 메모리로 치른다. 앱은
계열당 150개(`Values` 하나의 API 상한)에서 오래된 것부터 버린다.

시간 초과(503)로 끊긴 요청도 **끝난 단계까지는** 남는다. 합계는 적지 않는다 - 그 값은 언제나
상한이라 분포를 "얼마나 걸리는가"가 아니라 "언제 포기하는가"로 오염시킨다.
