...
- 계약 테스트: [`infra/privacy/privacy.test.mjs`](../../infra/privacy/privacy.test.mjs) 11건, CI `edge-test` 잡에 결선 (`.github/workflows/test.yml`)
...
|---|---|---|---|
| 음성 녹음 | 분석이 끝나면 즉시 삭제 | `ai/app/tempstore.py:10-14` (컨텍스트 매니저가 성공·실패·예외·취소 모두 `finally`에서 삭제), KAN-27 | `음성은 분석 직후 즉시 삭제라고 적혀 있다` |
...
| 음성 녹음 | DB·S3에 저장 안 함 | 2026-09-01 팀 회의 결정 (`infra/privacy/README.md`에 기록), 엔티티에 오디오 컬럼 없음 (`TestSession`·`TestResult`·`VocabAnswer`) | — |
| 익명 테스트 세션 | 토큰은 해시값만 저장 | `backend/src/main/java/app/accentury/backend/session/TestSession.java:36,45-46`, KAN-9 | — |
| 익명 테스트 세션 | 토큰 수명 30분 | `backend/src/main/resources/application.yml:102` (`session.ttl: 30m`) | `임시 파일 청소 기준 30분이 적혀 있다`(같은 「30분」 문자열을 함께 붙든다) |
| 익명 테스트 세션 | 세션 생성 24시간 후 자동 삭제 | `application.yml:112` (`analysis.retention: 24h`), `TestSession.java:140-157` (완료 시 결과 수명까지 연장), `session/SessionService.java:360-368` (`purgeExpired`) | `세션·결과 보유 기간 24시간이 적혀 있다` |
| 테스트 결과·어휘 답안 | 세션과 같은 24시간 | `result/TestResultRetention.java:22,33-36` (`deleteByExpiresAtBefore` 주기 실행), KAN-25·KAN-15 | 위와 같음 |
| 익명 통계 | 합계값만, 세션·IP·개인 점수 없음 | `backend/.../analytics/DailyCounter.java:33`, `AnalyticsCounters.java:45`, `result/CompletionService.java:132`, KAN-106 | — |
| 익명 통계 | 재응시·만료로 되돌리지 않음 | `DailyCounter.java:33`, `AnalyticsCounters.java:45` (둘 다 "되돌리지 않는다"를 명시) | — |
| 접속 IP | 메모리에서만 세고 저장 안 함 | `common/FixedWindowRateLimiter.java:9,12,20` (`ConcurrentHashMap` 인메모리 윈도우), `common/ClientIps.java:52-58` (판정만 하고 반환) | — |
| 서버 운영 로그 | 14일 | `infra/modules/fargate/variables.tf:173-176` (`log_retention_days` 기본값 14) | — |
| 보안 로그 | 7일, 인증 헤더는 가림 | `infra/modules/waf/variables.tf:21-24` (기본값 7), `infra/modules/waf/main.tf:286,313,319` (샘플 저장 끔 + `redacted_fields`) | — |
| 이용 통계 이벤트 | 보존은 GA 설정에 따름 (기본값 2개월) | **미확인** — 콘솔 기본값이고 레포에 근거가 없다 (7항 참조) | — |
| 비정상 종료 로그 | 90일 | **미확인** — Crashlytics 콘솔 기본값이고 레포에 근거가 없다 (7항 참조). 수집 항목 자체는 KAN-33, `docs/wiki/analytics.md` §8 | — |
...
`session/SessionService.java:28,87-88,201-203,245-247`과 `result/TestResultRepository.java:28`,
`vocab/VocabAnswerRepository.java:32`, `analysis/AnalysisJobRepository.java:98`이 근거이며
KAN-107 소관이다. 「캠페인 코드 64자 이하」는 `session/CreateSessionRequest.java`가 붙든다.
...
| 2. 제3자 제공 | 제공받는 자는 **확정 후 기재** (277행) | 사업자 미정. prod 게이트 (4항 참조) | — |
...
| 4. 국외 이전 | 이전 대상은 이용 통계와 오류 로그뿐 | 음성·세션·결과는 서울 리전 (`terraform.tfvars`), Firebase만 국외 | `데이터 소재지가 서울 리전이라고 적혀 있다` |
| 4. 국외 이전 | 광고 사업자 국외 이전은 **확정 후 기재** (344~346행) | 사업자 미정. prod 게이트 | — |
...
| 4. 이용 통계 이벤트 | 세션 id·토큰·문항·점수 원값을 싣지 않음 | `docs/wiki/analytics.md` §3 「익명 규칙」, `events.ts` | — |
...
| 5. 파기 | 파기 시점 네 가지 | 1항의 근거를 그대로 반복한다 (음성 즉시·30분, 세션 24시간, 재응시 즉시, 로그 14일/7일) | `음성은 분석 직후…`·`임시 파일 청소 기준 30분…`·`세션·결과 보유 기간 24시간…` |
| 6. 정보주체 권리 | 특정 이용자의 정보를 지목할 수단이 없음 | 계정 없음, 세션은 익명 (`TestSession.java` — 사람을 가리키는 컬럼 없음) | — |
...
| 8. 자동 수집 장치 | 진행 기록은 브라우저 저장소 | `web/src/progress/progressSnapshot.ts:9,34` (`localStorage`, 키 `accentury:progress`) | — |
| 8. 자동 수집 장치 | 세션 토큰·응시 키는 탭 저장소 | `web/src/session/webSession.ts`, `web/src/analytics/testId.ts:21-23` | — |
| 8. 자동 수집 장치 | 광고 SDK가 광고 식별자를 사용 | 2026-09-07 팀 결정, KAN-196. **아직 배선 없음** | — |
| 9. 공유 기능 | payload에 점수·세션·음성이 없음 | `app/src/main/java/com/accentury/app/bridge/SharePayload.kt:28-31` (필드는 `imageUrl`·`text`·`webTestUrl` 셋), `web/src/share/shareResult.ts:52,57` (KAN-30 요구) | — |
...
| 10. 광고 | 사업자·동의 UI 위치·국외 이전은 **확정 후 기재** (462·493·503~504행) | 사업자 미정. prod 게이트 | — |
| 11. 안전성 확보 | HTTPS, 토큰 해시, 임시 파일 최소 권한, 로그 비식별, WAF, 관리자 토큰 | `TestSession.java:45-46`, `ai/app/tempstore.py:6-16`, `infra/modules/waf/main.tf:313,319`, `common/AccenturyProperties.java:29` (admin 시크릿) | — |
...
| 14. 시행일 | 정식 게시일에 기재 | prod 게이트 (102·550행) | — |
...
| # | 자리 | 본문 행 | 닫는 조건 |
|---|---|---|---|
| 1 | 2항 제3자 제공 표 「광고 사업자 (확정 후 기재)」 | 277 | 광고 사업자 확정 (KAN-196) |
| 2 | 4항 광고 국외 이전 「(확정 후 기재)」 | 344~346 | 사업자 확정 + 소재 국가·항목·기간 확인 |
| 3 | 10항 「광고 사업자(확정 후 기재)」 | 462 | 사업자 확정 |
| 4 | 10항 동의 설정 위치 「(도입 시 위치 확정)」 | 493 | 앱 광고 동의 UI 구현 (KAN-196) |
| 5 | 10항 「4항과 이 항, 2항의 제3자 제공 표를 함께 채웁니다 (확정 후 기재)」 | 503~504 | 위 1~4가 닫히면 이 문장도 확정 문장으로 교체 |
| 6 | 시행일 「정식 게시일에 기재합니다」 | 102, 550 | 게시 당일 날짜로 교체 (두 자리 모두) |
...
| 2026-09-07 | 시행일은 초안 날짜가 아니라 정식 게시일에 기재한다 | 102·550행 (prod 게이트) |
